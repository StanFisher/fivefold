...
import { X, PlusCircle, MinusCircle, DollarSign, Calendar, Users, AlertCircle } from 'lucide-react';
...
export function TransactionModal({
  ...
  }, [defaultChildId, defaultType, childrenList]);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl pl-7 pr-3 py-2.5 text-white font-semibold text-base focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono"
                  placeholder="0.00"
                  required
                  autoFocus
                />
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3.5 py-2 text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 p-2.5 rounded-xl border text-left transition-all ${
                        selectedChildId === child.id
                          ? 'bg-indigo-950/60 border-indigo-500 text-white ring-1 ring-indigo-500 shadow-sm'
              ...
              <div className="p-3 bg-slate-800/60 border border-slate-700/60 rounded-xl space-y-1.5">
                ...
                <div className="flex gap-1.5 pt-1">
                  {childrenList.map((c) => (
                    <div
                      key={c.id}
                      className="flex-1 text-center py-1 rounded bg-slate-900/60 text-[10px] text-slate-300 truncate"
                      title={c.name}
                    >
                      {c.name}
                    </div>
            ...
            {allocationMode === 'split_custom' && (
              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {childrenList.map((child) => (
                  <div
                    key={child.id}
                    className="flex items-center justify-between gap-3 bg-slate-800/60 border border-slate-700/60 p-2 rounded-xl"
                  >
                    ...
                    <div className="relative w-28">
                      ...
                      <input
                        ...
                        className="w-full bg-slate-900 border border-slate-700 rounded-lg pl-5 pr-2 py-1 text-white text-xs text-right font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        placeholder="0.00"
                      />
                    </div>
                  </div>
