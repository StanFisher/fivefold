...
import { X, Sparkles, CheckCircle2, AlertCircle, Calendar, TrendingUp, Info } from 'lucide-react';
      ...
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-800/40">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">Monthly Interest Distribution</h2>
        ...
        <div className="p-6 overflow-y-auto space-y-5 flex-1">
          {error && (
            <div className="p-3.5 rounded-xl bg-rose-950/60 border border-rose-800/80 flex items-center gap-2.5 text-rose-300 text-sm">
          ...
          {/* Month Navigator */}
          <div className="flex items-center justify-between bg-slate-800/70 border border-slate-700/70 rounded-xl p-3">
            <button
              type="button"
              onClick={() => handleMonthChange(-1)}
              className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-slate-700 text-slate-200 hover:bg-slate-600 transition-colors"
            >
              &larr; Previous Month
            </button>
          ...
          {loading ? (
            ...
            <>
              ...
              {preview.alreadyPosted ? (
                <div className="p-3.5 rounded-xl bg-blue-950/50 border border-blue-800/60 flex items-center gap-3 text-blue-300 text-sm">
                ...
                <div className="p-3 rounded-xl bg-amber-950/30 border border-amber-800/40 flex items-start gap-2.5 text-xs text-amber-200/90">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="relative w-28">
                      ...
                      <input
                        ...
                        className="w-full bg-slate-900 border border-slate-700 rounded-lg pl-5 pr-2 py-1 text-white text-xs text-right font-mono focus:outline-none focus:ring-1 focus:ring-amber-400"
                        placeholder="0.00"
                      />
                    </div>
                  </div>
                  ...
                  <table className="w-full text-left text-xs">
                    <thead className="bg-slate-800/80 text-slate-400 uppercase font-semibold border-b border-slate-800">
                      <tr>
                        ...
                        <th className="py-2.5 px-3 text-right text-amber-400">Interest Added</th>
                    ...
                    <tbody className="divide-y divide-slate-800/60 font-mono">
                      {preview.childAllocations.map((alloc) => {
                        ...
                        return (
                          <tr key={alloc.childId} className="hover:bg-slate-800/40 transition-colors">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-800 bg-slate-800/40">
          ...
          {!preview?.alreadyPosted && (
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm text-white bg-amber-600 hover:bg-amber-500 shadow-lg shadow-amber-600/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
